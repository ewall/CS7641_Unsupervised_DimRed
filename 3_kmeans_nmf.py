...
if __name__ == "__main__":
	...
	runs = (("data/creditcards_train.arff", "Credit Default", "d1", 8),
	        ("data/htru_train.arff", "Pulsar Detection", "d2", 5))

	for (fname, label, abbrev, best_k) in runs:
		X, y, feature_names = load_xformed_data(fname, path.join(PKL_DIR, abbrev + "_nmf.pickle"))
		...
		for metric in ('distortion', 'silhouette', 'calinski_harabasz'):
			...
			visualizer.finalize()
			plt.savefig(path.join(PLOT_DIR, abbrev + "_km-nmf_elbow_" + metric + ".png"), bbox_inches='tight')
			visualizer.show()
			plt.close()
		...
		df.to_pickle(path.join(PKL_DIR, abbrev + "_km-nmf.pickle"))
		...
		visualizer.fit(X)
		visualizer.finalize()
		plt.savefig(path.join(PLOT_DIR, abbrev + "_km-nmf_silhouette.png"), bbox_inches='tight')
		visualizer.show()
		plt.close()
		...
		plt.savefig(path.join(PLOT_DIR, abbrev + "_km-nmf_scatter.png"), bbox_inches='tight')
		plt.show()
		plt.close()
		...
		visualizer.finalize()
		plt.savefig(path.join(PLOT_DIR, abbrev + "_km-nmf_parallel.png"), bbox_inches='tight')
		visualizer.show()
		plt.close()
