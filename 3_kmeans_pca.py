...
if __name__ == "__main__":
	...
	for (fname, label, abbrev, best_k) in runs:
		...
		for metric in ('distortion', 'silhouette', 'calinski_harabasz'):
			...
			visualizer.finalize()
			plt.savefig(path.join(PLOT_DIR, abbrev + "_km-pca_elbow_" + metric + ".png"), bbox_inches='tight')
			visualizer.show()
			plt.close()
		...
		df.to_pickle(path.join(PKL_DIR, abbrev + "_km-pca.pickle"))
		...
		visualizer.fit(X)
		visualizer.finalize()
		plt.savefig(path.join(PLOT_DIR, abbrev + "_km-pca_silhouette.png"), bbox_inches='tight')
		visualizer.show()
		plt.close()
		...
		plt.savefig(path.join(PLOT_DIR, abbrev + "_km-pca_scatter.png"), bbox_inches='tight')
		plt.show()
		plt.close()
		...
		visualizer.finalize()
		plt.savefig(path.join(PLOT_DIR, abbrev + "_km-pca_parallel.png"), bbox_inches='tight')
		visualizer.show()
		plt.close()
