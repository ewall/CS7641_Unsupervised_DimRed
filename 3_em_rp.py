...
runs = (("data/creditcards_train.arff", "Credit Default", "d1", 12, 'full'),
        ("data/htru_train.arff", "Pulsar Detection", "d2", 2, 'full'))

for (fname, label, abbrev, best_k, best_type) in runs:
	X, y, feature_names = load_xformed_data(fname, path.join(PKL_DIR, abbrev + "_rp.pickle"))
	...
	plt.savefig(path.join(PLOT_DIR, abbrev + "_em-rp_bic.png"), bbox_inches='tight')
	plt.show()
	plt.close()
	...
	plt.subplots_adjust(hspace=.35, bottom=.02)
	plt.savefig(path.join(PLOT_DIR, abbrev + "_em-rp_ch.png"), bbox_inches='tight')
	plt.show()
	plt.close()
	...
	plt.subplots_adjust(hspace=.35, bottom=.02)
	plt.savefig(path.join(PLOT_DIR, abbrev + "_em-rp_silhouette.png"), bbox_inches='tight')
	plt.show()
	plt.close()
	...
	df.to_pickle(path.join(PKL_DIR, abbrev + "_em-rp.pickle"))
	...
	plt.savefig(path.join(PLOT_DIR, abbrev + "_em-rp_scatter.png"), bbox_inches='tight')
	plt.show()
	plt.close()
	...
	visualizer.finalize()
	plt.savefig(path.join(PLOT_DIR, abbrev + "_em-rp_parallel.png"), bbox_inches='tight')
	visualizer.show()
	plt.close()
